...
using namespace std;
  ...
  GridSlamProcessor::GridSlamProcessor(): m_infoStream(cout){
    ...
    period_ = 5.0;
    m_obsSigmaGain=1;
    m_resampleThreshold=0.5; //采样频率
    m_minimumScore=0.;
    cout<<"----------in gridslamprocessor.cpp--------------"<<endl;
  }
  ...
  GridSlamProcessor::GridSlamProcessor(const GridSlamProcessor& gsp) 
    ...
    //add by chenyf 2018-11-5
    cout<< "-----------param in gridslamprocessor------------------"<<endl;
    cout<< "m_linearThresholdDistance= "<< m_linearThresholdDistance<<endl;
    cout << "m_angularThresholdDistance= "<< m_angularThresholdDistance<<endl;
    cout << "-------------------------------------------------------"<<endl;
    //end by chenyf 2018-11-5
...
(double srr, double srt, double str, double stt){
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles)
  {
    ...
    //只有当机器人移动了一定的距离或经过了一定的时间后，才进行扫描
#if 0 //by chenyf, 0.5m ,0.426 弧度
    cout<<"---------in gridslamprocessor--------------------"<<endl;
    cout<<"m_linearThresholdDistance= "<<m_linearThresholdDistance<<
          " m_angularThresholdDistance="<<m_angularThresholdDistance<<endl;
    cout<<"-------------------------------------------------"<<endl;
#endif
    if (! m_count 
        ...
        || (period_ >= 0.0 && (reading.getTime() - last_update_time_) > period_))
    {
      ...
      if (m_count>0)
      {
        ...
        //是gridslamprocessor.hxx 中的inline 函数
