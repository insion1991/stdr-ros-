...
{//熵发布器
  ...
  //scan_filter_ 是转换成相对odom坐标系的数据 
  scan_filter_->registerCallback(boost::bind(&SlamGMapping::laserCallback, this, _1));
  /*发布转换关系的线程，发布map to odom 的转换关系*/
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  ...
  //把ranges_double 中的东西读到gsp_laser_ 中
  ...
  //但是它在RangeReading中复制了它们，所以我们不需要保留我们的数组
...
/*
* 在 processScan 函数里依次执行了,运动模型，扫描匹配，权重更新，重采样，地图更新
*其中扫描匹配：在/opt/ros/kinetic/include/gmapping/gridfastslam/gridslamprocessor.hxx 的
             GridSlamProcessor::scanMatch 函数中。
*主要参照https://blog.csdn.net/roadseek_zw/article/details/53316177
*/
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  //500
  //ROS_WARN("****in slam_gmapping recive scan num %d",scan->ranges.size());

  // We can't initialize the mapper until we've got the first scan
/*一些重要参数的初始化，将slam里的参数传递到 openslam 里 ，设定坐标系，坐标原点，
 *以及采样函数随机种子的初始化，等等
...
*/
  if(addScan(*scan, odom_pose))
  {
    ...
    ROS_WARN("****in slam_gmapping after filter, scan num %d",scan->ranges.size());
...
/* 取最优粒子，根据权重和weightSum 判断（最大） */
  ...
  ROS_WARN("***in slam_gmapping updataMap,occ_thresh_=%f",occ_thresh_);
  /*确定地图的未知区域、自由区域、障碍 */
